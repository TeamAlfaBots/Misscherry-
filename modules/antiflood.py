...
async def cmd_setflood(client: Client, message: Message):
    ...
    if len(args) < 2:
        return await message.reply("Usage: <code>/setflood <number/off></code>")
    val = args[1].lower()
    if val in ("off", "no", "0"):
...
async def cmd_setfloodtimer(client: Client, message: Message):
    ...
    if len(args) < 2:
        return await message.reply("Usage: <code>/setfloodtimer <count> <duration></code> or <code>/setfloodtimer off</code>")
    ...
    if len(args) < 3:
        return await message.reply("Usage: <code>/setfloodtimer <count> <duration></code>")
    count = int(args[1]) if args[1].isdigit() else 0
    duration = parse_time(args[2])
    if not count or not duration:
        return await message.reply("❌ Invalid format. Example: <code>/setfloodtimer 10 30s</code>")
