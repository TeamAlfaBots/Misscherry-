...
@Client.on_message(filters.command("antiraid") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("raidtime") & ~filters.private & admin_filter)
async def cmd_raidtime(client: Client, message: Message):
    ...
    if not secs:
        return await message.reply("❌ Invalid time. Example: <code>6h</code>")
    await set_raidtime(message.chat.id, secs)
    await message.reply(await _(message.chat.id, "antiraid.raidtime_set", time=args[1]))


@Client.on_message(filters.command("raidactiontime") & ~filters.private & admin_filter)
async def cmd_raidactiontime(client: Client, message: Message):
    ...
    secs = parse_time(args[1])
    if not secs:
        return await message.reply("❌ Invalid time. Example: <code>1h</code>")
...
@Client.on_message(filters.command("autoantiraid") & ~filters.private & admin_filter)
