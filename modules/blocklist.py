...
async def cmd_addblocklist(client: Client, message: Message):
    ...
    if len(args) < 2:
        return await message.reply('Usage: <code>/addblocklist <trigger> [reason]</code>')
...
@Client.on_message(filters.command("unblocklistall") & ~filters.private)
...
async def cmd_list_blocklist(client: Client, message: Message):
    ...
    lines = [f"• <code>{i['trigger']}</code>" + (f" — {i['reason']}" if i.get("reason") else "") for i in items]
