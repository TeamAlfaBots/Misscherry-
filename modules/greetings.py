...
async def cmd_setwelcome(client: Client, message: Message):
    args = message.text.split(None, 1)
    if len(args) < 2:
        return await message.reply("Usage: <code>/setwelcome <text></code>\nFillings: <code>{mention}</code> <code>{first}</code> <code>{username}</code> <code>{chatname}</code> <code>{id}</code>")
...
async def cmd_setgoodbye(client: Client, message: Message):
    args = message.text.split(None, 1)
    if len(args) < 2:
        return await message.reply("Usage: <code>/setgoodbye <text></code>")
...
async def greet_handler(client: Client, update: ChatMemberUpdated):
    ...
    if old in ("left", "banned") and new in ("member", "restricted"):
        ...
        try:
            sent = await client.send_message(chat.id, text)
