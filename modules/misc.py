...
@Client.on_message(filters.command("rules") & ~filters.private)
...
@Client.on_message(filters.command("setrules") & ~filters.private & admin_filter)
async def cmd_setrules(client: Client, message: Message):
    args = message.text.split(None, 1)
    if len(args) < 2:
        return await message.reply("Usage: <code>/setrules <text></code>")
    await set_rules(message.chat.id, args[1])
    await message.reply(await _(message.chat.id, "rules.set"))


@Client.on_message(filters.command("resetrules") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("privaterules") & ~filters.private & admin_filter)
...
async def cmd_pinned(client: Client, message: Message):
    ...
    await message.reply(
        f'📌 <a href="https://t.me/c/{cid}/{chat.pinned_message.id}">Jump to pinned message</a>'
    )


@Client.on_message(filters.command("pin") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("unpin") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("unpinall") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("antichannelpin") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("cleanlinked") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command(["purge", "spurge"]) & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("del") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("purgefrom") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("purgeto") & ~filters.private & admin_filter)
...
@Client.on_message(filters.command("reports") & ~filters.private & admin_filter)
...
async def cmd_report(client: Client, message: Message):
    ...
    async for m in client.get_chat_members(message.chat.id, filter="administrators"):
        if not m.user.is_bot:
            name = escape(m.user.first_name or "Admin")
            admins.append(f'<a href="tg://user?id={m.user.id}">{name}</a>')
    await message.reply(
...
@Client.on_message(filters.command("setlang") & ~filters.private & admin_filter)
...
async def cmd_getlink(client: Client, message: Message):
    ...
    for chat in chats:
        try:
            ...
            lines.append(
                f"<b>{count}.</b>\n"
                f"Chat: <code>{chat_obj.title}</code>\n"
