...
async def cmd_warnmode(client: Client, message: Message):
    ...
    if len(args) < 2:
        s = await get_warn_settings(message.chat.id)
        return await message.reply(f"Current: <b>{s['mode']}</b>")
...
async def cmd_warnlimit(client: Client, message: Message):
    ...
    if len(args) < 2:
        s = await get_warn_settings(message.chat.id)
        return await message.reply(f"Current limit: <b>{s['limit']}</b>")
...
async def cmd_warntime(client: Client, message: Message):
    ...
    if len(args) < 2:
        s = await get_warn_settings(message.chat.id)
        return await message.reply(f"Current expiry: <b>{s['time'] or 'Never'}</b>")
    val = args[1].lower()
    if val in ("off", "no"):
    ...
    if not parse_time(val):
        return await message.reply("❌ Invalid time. Example: <code>4w</code> <code>3d</code> <code>1h</code>")
